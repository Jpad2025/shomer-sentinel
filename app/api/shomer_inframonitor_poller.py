"""
Inframonitor -- motor del poller: DB, sondeo de red/SNMP y el ciclo que corre
cada 30s (fast) / 300s (SNMP) contra switches, camaras, impresoras y POS.

12 sep 2026: separado de shomer_inframonitor.py (2.358 lineas mezclando DB,
sondeo y endpoints HTTP en un solo archivo). La particion se calculo con la
clausura transitiva real de referencias (que funcion usa que nombre, via AST,
incluyendo referencias pasadas como argumento -- asyncio.to_thread(func) no
es una "llamada" directa y un primer intento a mano se lo salto). Solo 4
nombres cruzan la frontera: _init_tables, _sync_guardian_aps y _ping (que
shomer_inframonitor.py importa de aca) y start_inframonitor_poller
(re-exportado desde alla para que main.py no cambie su import).
"""
import asyncio
import json
import logging
...
from app.api.shomer_pulse_correlate import write_poll_context
